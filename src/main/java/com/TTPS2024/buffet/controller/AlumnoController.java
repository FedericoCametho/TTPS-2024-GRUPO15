...
public class AlumnoController {
    ...
    @GetMapping("/listarPorDni")
    public ResponseEntity<Alumno> getAlumnosByDni(@RequestParam Integer dni){
    ...
    public ResponseEntity<Alumno> getAlumnoByEmail(@RequestParam String email){
        Alumno alumno = this.alumnoService.getUserByEmail(email);
        return (alumno != null) ? new ResponseEntity<>(alumno, HttpStatus.OK) : new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }

    @GetMapping("/listarPorNombre")
    public ResponseEntity<List<Alumno>> getAlumnosByName(@RequestParam String nombre){
        return new ResponseEntity<>(this.alumnoService.getUsersByName(nombre), HttpStatus.OK);
    }

    @GetMapping("/listarPorApellido")
    public ResponseEntity<List<Alumno>> getAlumnosByLastName(@RequestParam String apellido){
